...
### 8.1 采集配置显示「认证失败」或「超时」

- **原因**：与 N1 SSH 采集相同（SSH 凭据/网络问题）。
- **解决**：先检查 N1 采集（「采集」按钮）是否成功；N2 配置备份复用同一 SSH 凭据与 host key。

### 8.2 提示「配置内容未变化」

- **原因**：SHA-256 全文哈希相同，属于去重设计。
- **正常**：该设备配置自上次备份未变，不会产生冗余快照。若认为配置已变，检查设备侧是否真的修改过配置。

### 8.3 最新配置里看不到密码/密钥

- **原因**：`redact_config` 将 password/secret/community 值替换为 `********`。
- **正常**：N2 只存储脱敏后配置，明文密钥不落库。diff 同样只比较脱敏文本，密钥变化不会被展示（安全优先）。